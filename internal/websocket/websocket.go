...
type Websocket struct {
	...
	connHandler  ConnectionHandler  // 连接成功后的回调处理器
	logger       Logger             // 日志记录器
...
func NewWebsocket(dialURL string, handler Handler) *Websocket {
...
// SetConnectionHandler 设置连接成功后的回调处理器
func (m *Websocket) SetConnectionHandler(handler ConnectionHandler) {
...
func (m *Websocket) connect(dialURL string) error {
	...
	if m.connHandler != nil {
		if err := m.connHandler(m); err != nil {
			m.logger.Errorf("Connection handler error: %v", err)
			// 连接回调失败，关闭连接
			conn.Close()
			return fmt.Errorf("connection handler failed: %v", err)
		}
		m.logger.Debugf("Connection handler executed successfully")
	}
