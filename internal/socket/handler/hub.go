...
type Hub struct {
	...
	// 用户ID到连接ID的映射（一个用户可能有多个连接）
	userConnections map[string]map[string]bool // 用户ID -> 连接ID集合
	userMutex       sync.RWMutex               // 保护用户连接映射的读写锁
...
func (h *Hub) Start() error {
	...
	h.logger.Info("WebSocket 服务启动成功")
	return nil
}

// Stop 停止 Hub
func (h *Hub) Stop() {
	h.logger.Info("WebSocket 服务停止中...")
	...
	h.logger.Info("WebSocket 服务停止成功")
}
...
func (h *Hub) AddConnection(connID string, wsConn *websocket.Conn, metadata map[string]interface{}) (*Connection, error) {
	...
	conn := &Connection{
		ID:       connID,
		UserID:   "", // 初始为空，后续可通过 BindUserID 绑定
		Conn:     wsConn,
		Metadata: metadata,
		Created:  time.Now(),
		LastSeen: time.Now(),
	...
	h.logger.Info("添加WebSocket连接成功", "conn_id", connID)
	return conn, nil
}
...
func (h *Hub) RemoveConnection(connID string) error {
	...
	h.logger.Info("移除WebSocket连接成功", "conn_id", connID)
	...
	if userID != "" {
		h.userMutex.Lock()
		if connSet, exists := h.userConnections[userID]; exists {
			delete(connSet, connID)
			if len(connSet) == 0 {
				// 如果该用户没有其他连接了，删除用户映射
...
// BindUserID 绑定用户ID到连接
func (h *Hub) BindUserID(connID string, userID string) error {
	if userID == "" {
	...
	// 更新用户连接映射
	h.userMutex.Lock()
	// 如果之前绑定了其他用户ID，先清理旧的绑定
	if oldUserID != "" && oldUserID != userID {
		if connSet, exists := h.userConnections[oldUserID]; exists {
			delete(connSet, connID)
			if len(connSet) == 0 {
	...
	if h.userConnections[userID] == nil {
		h.userConnections[userID] = make(map[string]bool)
	}
	h.userConnections[userID][connID] = true
	h.userMutex.Unlock()

	h.logger.Info("绑定用户ID到WebSocket连接成功", "conn_id", connID, "user_id", userID)
	return nil
}

// GetConnectionsByUserID 根据用户ID获取所有连接的ID列表
func (h *Hub) GetConnectionsByUserID(userID string) []string {
	if userID == "" {
		return nil
	}

	h.userMutex.RLock()
	connSet, exists := h.userConnections[userID]
	if !exists {
		h.userMutex.RUnlock()
		return nil
	}
...
// SendMessageToUser 发送消息到指定用户的所有连接
func (h *Hub) SendMessageToUser(userID string, message []byte) error {
	if userID == "" {
		return fmt.Errorf("用户ID不能为空")
	}

	connIDs := h.GetConnectionsByUserID(userID)
	if len(connIDs) == 0 {
		return fmt.Errorf("没有找到活跃的连接: %s", userID)
	}

	var errors []error
	for _, connID := range connIDs {
		if err := h.SendMessage(connID, message); err != nil {
			errors = append(errors, fmt.Errorf("发送消息失败: %s: %w", connID, err))
		}
	}
...
func (h *Hub) Broadcast(message []byte) {
	...
	default:
		h.logger.Warn("WebSocket 广播通道已满，丢弃消息")
	}
}
		...
		go func(c *Connection) {
			...
			if c.Conn != nil {
				...
				if err := c.Conn.WriteMessage(websocket.TextMessage, message); err != nil {
					h.logger.Warn("WebSocket 发送广播消息失败", "conn_id", c.ID, "error", err.Error())
					// 记录失败的连接，稍后统一移除
					failedMutex.Lock()
...
func (h *Hub) GetConnectionInfo(connID string) (*ConnectionInfo, error) {
	conn, exists := h.GetConnection(connID)
	if !exists {
		return nil, fmt.Errorf("WebSocket 连接不存在: %s", connID)
	}
...
func (h *Hub) listenConnection(conn *Connection) {
	defer func() {
		...
		if _, exists := h.connections[conn.ID]; exists {
			...
			h.logger.Info("WebSocket 连接断开", "conn_id", conn.ID)
		}
		h.connMutex.Unlock()
	}()
	...
	for {
		...
		default:
			...
			if err != nil {
				h.logger.Error("WebSocket 读取消息失败", "conn_id", conn.ID, "error", err.Error())
				return
			}
...
func (h *Hub) hubMessageHandler(connID string, message []byte) {
	...
	if h.config.MaxMessageSize > 0 && len(message) > h.config.MaxMessageSize {
		h.logger.Warn("WebSocket 消息太大", "conn_id", connID, "size", len(message), "limit", h.config.MaxMessageSize)
		return
	}
